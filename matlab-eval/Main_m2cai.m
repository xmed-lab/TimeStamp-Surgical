...
phaseGroundTruths = {};
gt_root_folder = '/datasets/m2cai16/annotation_folder';
for k = 1:14
    %num = num2str(k);
    if k<10
        to_add = ['workflow_video_' num2str(k,'%02d');];
    end
    
    if k>9
        to_add = ['workflow_video_' num2str(k);];
    end
...
for i = 1:length(phaseGroundTruths)
    ...
    predFile = [predroot phaseGroundTruth(end-5:end-4) '_pred.txt'];
    disp(predFile)
